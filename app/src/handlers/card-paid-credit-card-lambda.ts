import middy from "@middy/core";
import type { APIGatewayEvent, APIGatewayProxyResult } from "aws-lambda";
import { v4 as uuid } from "uuid";
import { DynamoDBService } from "../services/dynamodb/dynamodb.service.js";
import { SimpleQueueService } from "../simple-queue-service/simple-queue.service.js";

interface IPaidCreditPayload {
  merchant: string;
  amount: number;
}

interface IPaidCreditTransactionDB extends IPaidCreditPayload {
  uuid: string;
  cardId: string;
  type: string;
  createdAt: string;
}

const cardPaidCreditHandler = async (
  event: APIGatewayEvent
): Promise<APIGatewayProxyResult> => {
  try {
    const body = JSON.parse(event.body || "{}");
    const { merchant, amount }: IPaidCreditPayload = body;
    const cardId = event.pathParameters?.cardId;

    if (!cardId) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: "cardId is required" }),
        headers: { "Content-type": "application/json" }
      };
    }

    if (!merchant || !amount) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: "merchant and amount are required" }),
        headers: { "Content-type": "application/json" }
      };
    }

    if (amount <= 0) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: "Amount must be greater than 0" }),
        headers: { "Content-type": "application/json" }
      };
    }

    const dynamoDBService = new DynamoDBService();
    const sqsService = new SimpleQueueService();
    const CARDS_TABLE_NAME = process.env.DYNAMODB_CARDS_TABLE || "card-table";
    const TRANSACTION_TABLE_NAME = process.env.DYNAMODB_TRANSACTION_TABLE || "";
    const QUEUE_URL = process.env.NOTIFICATIONS_EMAIL_SQS_URL || "";

    const cardResponse = await dynamoDBService.getItem({
      TableName: CARDS_TABLE_NAME,
      Key: {
        uuid: cardId
      }
    });

    if (!cardResponse.Item) {
      return {
        statusCode: 404,
        body: JSON.stringify({ error: "Card not found" }),
        headers: { "Content-type": "application/json" }
      };
    }

    const card = cardResponse.Item;

    if (card.type !== "CREDIT") {
      return {
        statusCode: 400,
        body: JSON.stringify({ 
          error: "Payment only allowed for credit cards",
          cardType: card.type
        }),
        headers: { "Content-type": "application/json" }
      };
    }

    if (card.status !== "ACTIVATED") {
      return {
        statusCode: 400,
        body: JSON.stringify({ 
          error: "Card is not activated",
          cardStatus: card.status,
          message: "Only activated cards can process payments"
        }),
        headers: { "Content-type": "application/json" }
      };
    }

    const currentBalance = card.balance || 0;

    const newBalance = currentBalance + amount;

    await dynamoDBService.updateItem({
      TableName: CARDS_TABLE_NAME,
      Key: {
        uuid: cardId
      },
      UpdateExpression: "SET balance = :newBalance",
      ExpressionAttributeValues: {
        ":newBalance": newBalance
      }
    });

    const transactionPayload: IPaidCreditTransactionDB = {
      uuid: uuid(),
      cardId,
      amount,
      merchant: "PSE",
      type: "PAYMENT_BALANCE",
      createdAt: new Date().toISOString(),
    };

    await dynamoDBService.saveItem({
      TableName: TRANSACTION_TABLE_NAME,
      Item: transactionPayload,
    });

    await sqsService.sendMessage({
      queueUrl: QUEUE_URL,
      body: {
        type: "TRANSACTION.PAID",
        data: {
          date: new Date().toISOString(),
          merchant,
          amount: amount,
        },
      },
    });

    return {
      statusCode: 200,
      body: JSON.stringify({ 
        transaction: transactionPayload,
        cardBalance: {
          previous: currentBalance,
          current: newBalance,
          amountPaid: amount
        },
        message: "Credit payment processed successfully"
      }),
      headers: {
        "Content-type": "application/json",
      },
    };

  } catch (error) {
    console.error("Error processing credit payment:", error);
    return {
      statusCode: 500,
      body: JSON.stringify({ 
        error: "Internal server error",
        message: error instanceof Error ? error.message : "Unknown error"
      }),
      headers: {
        "Content-type": "application/json",
      },
    };
  }
};

export const handler = middy<APIGatewayEvent, APIGatewayProxyResult>(
  cardPaidCreditHandler
);